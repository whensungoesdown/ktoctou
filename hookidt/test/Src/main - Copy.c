...
LONG HandleSmap(VOID)
{
	...
	else if (((ULONG)ulEip < 0x80000000) && (error & PF_USER) && (pPte->u.Hard.Owner == 0))
	{
		...
		if ((ULONG)ulEip == 0x7c90e4f4   // ntdll!KiFastSystemCallRet
			...
			|| (ULONG)ulEip == 0x7c8106eb // read user stack
		...
		else
		{
			...
			if (error & PF_WRITE)
			{
				...
				time.QuadPart = (LONGLONG)-10000; // waits for 1 milliseconds

				while (TRUE)
				{
					KeDelayExecutionThread(UserMode, TRUE, &time);

					if (IsSmapPage(0, (ULONG)VirtualAddress))
					//if (0 == pPte->u.Hard.Owner)
					{
						__asm int 3;
						//DbgPrint("after wait 50 miliiseconds, Write Access! Deny! Change to READ_ONLY\n"); // process switch could happen during DbgPrint
					...
					else
					{
						...
						DbgPrint("Page released, writting!\n");
						return 1;
					}
				}
